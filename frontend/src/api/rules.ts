...
import type { Rule, EvalResult, RuleCreateResponse, RuleToggleResponse, RulePreviewItem, PaginatedResponse } from "../types";
...
export const rulesApi = {
  ...
  create: (params: Record<string, unknown>, signal?: AbortSignal) =>
    post<RuleCreateResponse>("/rules", null, { params, signal }),
  ...
  toggle: (ruleId: string, enabled: boolean, signal?: AbortSignal) =>
    post<RuleToggleResponse>(`/rules/${ruleId}/toggle?enabled=${enabled}`, undefined, { signal }),
  ...
  preview: (signal?: AbortSignal) =>
    get<RulePreviewItem[]>("/rules/preview", { signal }),
};
