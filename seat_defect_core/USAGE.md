...
- 每个机位至少 50 张，推荐 100-200 张；高召回训练建议 500+ 张
- 图像应为该机位原始输入图；训练脚本会复用当前检测配置的 YOLO+ROI 前置流程，自动生成与线上 EfficientAD 输入一致的 aligned ROI 训练图
- 训练产物旁会保存 `<model_stem>_prepared_roi/`，用于复查实际参与训练的 ROI 图和 mask
